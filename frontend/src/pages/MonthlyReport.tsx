  ...
  return (
    <section className="page">
      ...
      <div className="report-chart-grid print-friendly">
        <div className="panel chart-panel"><h3>أداء الفنادق — صافي الإيراد</h3><ResponsiveContainer width="100%" height={285}><BarChart data={rows.slice(0, 12)} margin={{ top: 12, right: 16, left: 8, bottom: 12 }}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="hotel_name" tick={{ fontSize: 10 }} interval={0} angle={-20} textAnchor="end" height={55}/><YAxis/><Tooltip/><Bar dataKey="net_revenue" name="صافي الإيراد" fill="#23698e" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer></div>
        <div className="panel chart-panel"><h3>أداء الفنادق — الحجوزات</h3><ResponsiveContainer width="100%" height={285}><BarChart data={rows.slice(0, 12)} margin={{ top: 12, right: 16, left: 8, bottom: 12 }}><CartesianGrid strokeDasharray="3 3"/><XAxis dataKey="hotel_name" tick={{ fontSize: 10 }} interval={0} angle={-20} textAnchor="end" height={55}/><YAxis allowDecimals={false}/><Tooltip/><Bar dataKey="bookings" name="الحجوزات" fill="#4d91b6" radius={[5,5,0,0]}/></BarChart></ResponsiveContainer></div>
      </div>
