...
import { createReview, decideReview, getEmployees, getHotels, getPlatforms, getReviews, Hotel, Platform, User } from '../api'
...
export default function Reviews({ user }: { user: User }) {
  ...
  const load = () => getReviews().then(setRows)
  ...
  async function save(e: FormEvent) {
    ...
    try {
      ...
      setMsg('تم حفظ التقييم وإرساله للمراجعة')
      setForm({ ...form, booking_number: '', comment: '', proposed_action: '' })
      load()
    } catch (ex: any) { setMsg(ex.message) }
  }
  async function decision(id: number, status: string) { try { await decideReview(id, status); load() } catch (ex: any) { setMsg(ex.message) } }

  return <section className="page">
    <div className="page-head"><div><h2>إدارة التقييمات والمراجعات</h2><p>الموظف يدخل التقييم والإجراء المقترح، مع تحديد المنصة، والمدير يعتمد أو يرفض.</p></div><div className="actions no-print"><PrintButton /></div></div>
    <form className="form-card no-print" onSubmit={save}><div className="form-grid">
      ...
      <label className="span-2">التعليق<textarea required value={form.comment} onChange={e => setForm({ ...form, comment: e.target.value })} /></label>
    ...
    </div><button className="btn primary">حفظ التقييم</button>{msg && <span className="inline-msg">{msg}</span>}</form>
    <div className="panel print-friendly"><div className="table-wrap"><table><thead><tr><th>التاريخ</th><th>الحجز</th><th>الفندق</th><th>المنصة</th><th>التقييم</th><th>المشاعر</th><th>التعليق</th><th>الإجراء المقترح</th><th>الموظف</th><th>الحالة</th><th className="no-print">قرار</th></tr></thead><tbody>{rows.map(r => <tr key={r.id}><td>{r.review_date}</td><td>{r.booking_number}</td><td>{r.hotel_name}</td><td>{r.platform_name}</td><td>{r.rating}</td><td>{r.sentiment}</td><td>{r.comment}</td><td>{r.proposed_action}</td><td>{r.employee_name}</td><td><span className={`status ${r.status.toLowerCase()}`}>{r.status}</span></td><td className="no-print">{(user.role === 'manager' || user.role === 'admin') && r.status === 'Pending' ? <><button className="mini ok" onClick={() => decision(r.id, 'Approved')}>اعتماد</button><button className="mini bad" onClick={() => decision(r.id, 'Rejected')}>رفض</button></> : <span>—</span>}</td></tr>)}</tbody></table></div></div>
  </section>
}
